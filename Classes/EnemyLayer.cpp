...
bool EnemyLayer::init()
{
	...
	do 
	{
		...
		CCAnimationCache::sharedAnimationCache()->addAnimation(animation1,"Enemy1Blowup");//把初始化好的动画缓存起来
...
void EnemyLayer::addEnemy1(float dt)
{
	...
	enemy1->bindSprite(CCSprite::createWithSpriteFrame(enemy1SpriteFrame),ENEMY1_MAXLIFE);//使用前面init初始化的图片缓存
	...
	float minDuration,maxDuration;//飞机的速度
...
void EnemyLayer::enemy1Blowup(Enemy* enemy1)
{
	...
	CCAnimation* animation=CCAnimationCache::sharedAnimationCache()->animationByName("Enemy1Blowup");//从动画缓存中调取动画
